...
import { RFPStatus } from '@docflows/shared';
...
export interface PaymentFiltersState {
  ...
  sortBy: 'rfpNumber' | 'payee' | 'amount' | 'dateNeeded' | 'createdAt' | 'status';
  sortOrder: 'asc' | 'desc';
}
...
const sortOptions = [
  { label: 'RFP Number', value: 'rfpNumber' },
...
export default function PaymentFilters({
  ...
  const updateFilter = (key: keyof PaymentFiltersState, value: unknown) => {
  ...
  const statusOptions = useMemo(() => {
    const options = [{ label: 'All Statuses', value: null as RFPStatus | null }];
    Object.values(RFPStatus).forEach((status) => {
      options.push({
        label: getStatusLabel(status),
        value: status as RFPStatus,
      });
    });
    return options;
  }, []);
