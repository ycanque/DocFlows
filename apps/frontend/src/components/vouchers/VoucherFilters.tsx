...
import { CheckVoucherStatus } from '@docflows/shared';
...
export interface VoucherFiltersState {
  searchQuery: string;
  status: CheckVoucherStatus | null;
  createdFrom: string | null;
  createdTo: string | null;
  amountMin: string | null;
  amountMax: string | null;
  sortBy: 'cvNumber' | 'payee' | 'amount' | 'createdAt' | 'status';
  sortOrder: 'asc' | 'desc';
}
...
const sortOptions = [
  { label: 'CV Number', value: 'cvNumber' },
...
export default function VoucherFilters({
  ...
  const updateFilter = (key: keyof VoucherFiltersState, value: unknown) => {
  ...
  const statusOptions = useMemo(() => {
    const options = [{ label: 'All Statuses', value: null as CheckVoucherStatus | null }];
    Object.values(CheckVoucherStatus).forEach((status) => {
      options.push({
        label: getStatusLabel(status),
        value: status as CheckVoucherStatus,
      });
    });
    return options;
  }, []);
      ...
      <CardContent className="p-4 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by CV number, payee, or particulars..."
